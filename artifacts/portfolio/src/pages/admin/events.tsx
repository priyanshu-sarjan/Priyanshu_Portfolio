...
type Form = { title: string; description: string; date: string; type: EventType; completed: boolean };
const emptyForm: Form = { title: "", description: "", date: "", type: "task", completed: false };
...
export default function AdminEvents() {
  ...
  const openEdit = (e: typeof events[0]) => {
    setEditId(e.id);
    setForm({ title: e.title, description: e.description || "", date: e.date, type: e.type as EventType, completed: e.completed });
    setOpen(true);
  };

  const toggleComplete = async (e: typeof events[0]) => {
    await updateEvent.mutateAsync({ id: e.id, data: { title: e.title, description: e.description || null, date: e.date, type: e.type as EventType, completed: !e.completed } }, { onSuccess: refresh });
  };

  const handleSubmit = async () => {
    const data = { title: form.title, description: form.description || null, date: form.date, type: form.type, completed: form.completed };
    if (editId !== null) {
    ...
    <AdminLayout>
      <div className="p-8 space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold">Events & Tasks</h1>
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent>
            ...
            <div className="space-y-4">
              <div className="space-y-1"><label className="text-sm font-medium">Title</label><Input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} placeholder="e.g. IEEE Tech Summit" /></div>
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1"><label className="text-sm font-medium">Date</label><Input value={form.date} onChange={e => setForm(f => ({ ...f, date: e.target.value }))} placeholder="2025-06-20" /></div>
              </div>
